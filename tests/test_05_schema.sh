...
assert_code "3.1 非法 field_type → 42024" "42024" "$body"
...
assert_code "3.2 array 不合法 → 42024" "42024" "$body"

# =============================================================================
# 4. constraints 自洽校验 (42025)
# =============================================================================
...
# 4.4 select: empty options — 可能不校验，视实现而定
...
# =============================================================================
# 5. default_value vs constraints (42026)
# =============================================================================
...
assert_code "5.1 int default 超 max → 42026" "42026" "$body"
...
assert_code "5.2 int default 低于 min → 42026" "42026" "$body"
...
assert_code "5.3 string default 超 maxLength → 42026" "42026" "$body"
...
V=$(echo "$(post "/event-type-schema/list" "{}")" | jq -r ".data.items[] | select(.id==$SCHEMA_ID2) | .version" | tr -d '\r')
...
# 停用
V=$(echo "$(post "/event-type-schema/list" "{}")" | jq -r ".data.items[] | select(.id==$TMP_SCHEMA) | .version" | tr -d '\r')
...
assert_code "9.1 版本冲突 → 42030" "42030" "$body"

# =============================================================================
# 10. 编辑启用中 schema (42031)
# =============================================================================
...
assert_code_in "10.1 编辑启用中 schema" "42031 0" "$body"
...
if [ -n "$UNK_ID" ] && [ "$UNK_ID" != "null" ] && [ "$UNK_ID" != "" ]; then
  V=$(echo "$(post "/event-type-schema/list" "{}")" | jq -r ".data.items[] | select(.id==$UNK_ID) | .version" | tr -d '\r')
...
if [ -n "$NULL_ID" ] && [ "$NULL_ID" != "null" ] && [ "$NULL_ID" != "" ]; then
  V=$(echo "$(post "/event-type-schema/list" "{}")" | jq -r ".data.items[] | select(.id==$NULL_ID) | .version" | tr -d '\r')
...
if [ -n "$HUGE_ID" ] && [ "$HUGE_ID" != "null" ] && [ "$HUGE_ID" != "" ]; then
  V=$(echo "$(post "/event-type-schema/list" "{}")" | jq -r ".data.items[] | select(.id==$HUGE_ID) | .version" | tr -d '\r')
