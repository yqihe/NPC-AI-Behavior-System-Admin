...
fld_rm()         { fld_disable "$1"; post "/fields/delete" "{\"id\":$1}" > /dev/null 2>&1; }
...
tpl_rm()         { tpl_disable "$1"; post "/templates/delete" "{\"id\":$1}" > /dev/null 2>&1; }
...
assert_code "f2.13 创建 string" "0" "$R"
...
assert_code "f2.14 创建 boolean" "0" "$R"
...
assert_code "f2.15 创建 select" "0" "$R"

R=$(post "/fields/create" "{\"name\":\"${P}f_float\",\"label\":\"浮点字段\",\"type\":\"float\",\"category\":\"combat\",\"properties\":{\"description\":\"fl\",\"expose_bb\":false,\"constraints\":{\"min\":0.0,\"max\":100.0,\"precision\":2}}}")
FLOAT_ID=$(echo "$R" | jq -r '.data.id')
assert_code "f2.16 创建 float" "0" "$R"
...
# 把 ATK 启用准备给 reference 用
fld_enable "$ATK_ID"

# ---- 功能 10/11：收紧 + 循环引用 ----
subsection "功能 10/11: 约束收紧 + 循环引用"
...
assert_code "f10.3 min 收紧 40007" "40007" "$R"
...
assert_code "f10.4 max 收紧 40007" "40007" "$R"
...
assert_code "f11.2 引用停用字段 40013" "40013" "$R"
...
assert_code "f11.3 引用不存在字段 40014" "40014" "$R"

# 删除 reference 字段后 target.ref_count 回退
fld_rm "$REFONE_ID"
R=$(fld_detail "$TGT_ID")
assert_field "f11.4 删除引用方后 target ref_count=0" ".data.ref_count" "0" "$R"

# ---- 功能 7：字段引用详情（跨模块补 label 在 Part 4 验证）----
...
assert_ge         "f7.1 至少 1 个字段引用" ".data.fields | length" "1" "$R"
...
F_DISABLED=$(echo "$R" | jq -r '.data.id')   # 不启用
...
# description 超长（UTF-8 字符数 > 512）
LONG_DESC=$(python3 -c "print('长' * 513)" 2>/dev/null || python -c "print('长' * 513)" 2>/dev/null)
...
assert_field "t6.1 npcs 空" ".data.npcs | length" "0" "$R"
...
# ---- Attack 1: reference 嵌套（docs 说禁止，代码只查环不查嵌套）----
subsection "ATK-1: reference 嵌套 — 期望 40009 或新增嵌套拒绝码"
...
assert_code "atk1.1 refA -> LEAF1 成功" "0" "$R"
REF_A=$(echo "$R" | jq -r '.data.id'); fld_enable "$REF_A"

# 尝试创建 refB 引用 refA（即 reference 引用 reference）
...
# 期望：某种拒绝（建议新增错误码，或复用 40009 循环引用拒绝嵌套）
# 当前代码：会成功（只有 detectCyclicRef 查环，没查嵌套）
assert_bug  "atk1.2 reference 嵌套应被拒绝" "40016" "$R" "reference 字段 refs 允许指向另一个 reference 字段，应由 validateReferenceRefs 返回 40016 ErrFieldRefNested。"
REF_B=$(echo "$R" | jq -r '.data.id // empty')
# 如果真的创建成功，记下 id 以便清理
if [ -n "$REF_B" ] && [ "$REF_B" != "null" ]; then
  fld_enable "$REF_B" 2>/dev/null || true
fi

# ---- Attack 2: refs 数组含重复 ID ----
subsection "ATK-2: refs 数组重复 ID — 期望 40000/40009 或 DB unique 拒绝"
...
# 当前代码：syncFieldRefs 对 newRefIDs 做 set diff 但是对外层迭代没有 dedup，
# 若 field_refs 有 UNIQUE(field_id, ref_type, ref_id)，第二次 Add 会报错 → 50000；
# 若没 UNIQUE，则 IncrRefCount 执行两次 → leaf2.ref_count = 2
CODE=$(echo "$R" | jq -r '.code' | tr -d '\r')
TOTAL=$((TOTAL + 1))
if [ "$CODE" = "0" ]; then
  # 创建成功：检查 ref_count 是否被重复递增
  DUP_ID=$(echo "$R" | jq -r '.data.id')
  L2_RC=$(fld_refcount "$LEAF2")
  if [ "$L2_RC" = "1" ]; then
    echo "  [PASS] atk2.1 创建成功且 leaf2.ref_count=1（参数已被 DB unique 约束或代码去重）"
    PASS=$((PASS+1))
  else
    echo "  [BUG ] atk2.1 创建成功但 leaf2.ref_count=$L2_RC（应为 1）"
    FAIL=$((FAIL+1))
    BUGS+=("atk2.1: refs=[id,id] 未去重导致 ref_count 被重复递增为 $L2_RC")
  fi
  fld_rm "$DUP_ID"
elif [ "$CODE" = "40000" ] || [ "$CODE" = "40009" ]; then
  echo "  [PASS] atk2.1 重复 refs 被拒绝 (code=$CODE)"
  PASS=$((PASS+1))
else
  echo "  [BUG ] atk2.1 重复 refs 返回了非 0 / 40000 / 40009 的错误 code=$CODE"
  FAIL=$((FAIL+1))
  BUGS+=("atk2.1: refs=[id,id] 返回 code=$CODE，可能是 DB unique 触发的 50000 而非业务校验")
fi

# ---- Attack 3: select 收紧 minSelect/maxSelect 不检查 ----
subsection "ATK-3: select.minSelect 扩大（收紧）应被拒绝"

R=$(post "/fields/create" "{\"name\":\"${P}atk_sel\",\"label\":\"选择字段\",\"type\":\"select\",\"category\":\"basic\",\"properties\":{\"description\":\"\",\"expose_bb\":false,\"constraints\":{\"options\":[{\"value\":\"a\",\"label\":\"A\"},{\"value\":\"b\",\"label\":\"B\"},{\"value\":\"c\",\"label\":\"C\"}],\"minSelect\":1,\"maxSelect\":3}}}")
SEL_ID=$(echo "$R" | jq -r '.data.id')
fld_enable "$SEL_ID"

# 用另一个 reference 字段把它引用起来
R=$(post "/fields/create" "{\"name\":\"${P}atk_sel_holder\",\"label\":\"持有者\",\"type\":\"reference\",\"category\":\"basic\",\"properties\":{\"description\":\"\",\"expose_bb\":false,\"constraints\":{\"refs\":[${SEL_ID}]}}}")
HOLDER=$(echo "$R" | jq -r '.data.id')

R=$(fld_detail "$SEL_ID")
SEL_RC=$(echo "$R" | jq -r '.data.ref_count' | tr -d '\r')
if [ "$SEL_RC" != "1" ]; then
  echo "  [SETUP-FAIL] atk3 ref_count 期望 1 实际 $SEL_RC — 跳过"
else
  fld_disable "$SEL_ID"
  VER=$(fld_version "$SEL_ID")

  # minSelect 从 1 收紧到 2（原来选 1 个合法，现在不合法）
  R=$(post "/fields/update" "{\"id\":${SEL_ID},\"label\":\"选择字段\",\"type\":\"select\",\"category\":\"basic\",\"properties\":{\"description\":\"\",\"expose_bb\":false,\"constraints\":{\"options\":[{\"value\":\"a\",\"label\":\"A\"},{\"value\":\"b\",\"label\":\"B\"},{\"value\":\"c\",\"label\":\"C\"}],\"minSelect\":2,\"maxSelect\":3}},\"version\":${VER}}")
  assert_bug "atk3.1 minSelect 1→2 应 40007" "40007" "$R" "被引用 select 字段允许收紧 minSelect；service.checkConstraintTightened 的 case select 只查 options 集合，未比对 minSelect/maxSelect。"

  # maxSelect 从 3 收紧到 2
  VER=$(fld_version "$SEL_ID")
  R=$(post "/fields/update" "{\"id\":${SEL_ID},\"label\":\"选择字段\",\"type\":\"select\",\"category\":\"basic\",\"properties\":{\"description\":\"\",\"expose_bb\":false,\"constraints\":{\"options\":[{\"value\":\"a\",\"label\":\"A\"},{\"value\":\"b\",\"label\":\"B\"},{\"value\":\"c\",\"label\":\"C\"}],\"minSelect\":1,\"maxSelect\":2}},\"version\":${VER}}")
  assert_bug "atk3.2 maxSelect 3→2 应 40007" "40007" "$R" "被引用 select 字段允许收紧 maxSelect；同上。"

  # 验证 options 删除仍被拦截（对照组）
  VER=$(fld_version "$SEL_ID")
  R=$(post "/fields/update" "{\"id\":${SEL_ID},\"label\":\"选择字段\",\"type\":\"select\",\"category\":\"basic\",\"properties\":{\"description\":\"\",\"expose_bb\":false,\"constraints\":{\"options\":[{\"value\":\"a\",\"label\":\"A\"},{\"value\":\"b\",\"label\":\"B\"}],\"minSelect\":1,\"maxSelect\":2}},\"version\":${VER}}")
  assert_code "atk3.3 对照: 删除 options 40007" "40007" "$R"
fi

# ---- Attack 4: float.precision 降低不检查 ----
subsection "ATK-4: float.precision 降低（收紧）应被拒绝"

R=$(post "/fields/create" "{\"name\":\"${P}atk_fl\",\"label\":\"浮点收紧\",\"type\":\"float\",\"category\":\"basic\",\"properties\":{\"description\":\"\",\"expose_bb\":false,\"constraints\":{\"min\":0.0,\"max\":100.0,\"precision\":4}}}")
FL_ID=$(echo "$R" | jq -r '.data.id')
fld_enable "$FL_ID"

R=$(post "/fields/create" "{\"name\":\"${P}atk_fl_holder\",\"label\":\"fl持有\",\"type\":\"reference\",\"category\":\"basic\",\"properties\":{\"description\":\"\",\"expose_bb\":false,\"constraints\":{\"refs\":[${FL_ID}]}}}")
FL_HOLDER=$(echo "$R" | jq -r '.data.id')

fld_disable "$FL_ID"
VER=$(fld_version "$FL_ID")
R=$(post "/fields/update" "{\"id\":${FL_ID},\"label\":\"浮点收紧\",\"type\":\"float\",\"category\":\"basic\",\"properties\":{\"description\":\"\",\"expose_bb\":false,\"constraints\":{\"min\":0.0,\"max\":100.0,\"precision\":2}},\"version\":${VER}}")
assert_bug "atk4.1 precision 4→2 应 40007" "40007" "$R" "float 字段 precision 降低会丢失小数精度，但 checkConstraintTightened 未覆盖此 key。"

# ---- Attack 5: integer.step 变更不检查 ----
subsection "ATK-5: integer.step 变更不检查（可能不是 bug，仅记录）"

R=$(post "/fields/create" "{\"name\":\"${P}atk_step\",\"label\":\"step 字段\",\"type\":\"integer\",\"category\":\"basic\",\"properties\":{\"description\":\"\",\"expose_bb\":false,\"constraints\":{\"min\":0,\"max\":100,\"step\":1}}}")
STEP_ID=$(echo "$R" | jq -r '.data.id')
fld_enable "$STEP_ID"

R=$(post "/fields/create" "{\"name\":\"${P}atk_step_holder\",\"label\":\"step持有\",\"type\":\"reference\",\"category\":\"basic\",\"properties\":{\"description\":\"\",\"expose_bb\":false,\"constraints\":{\"refs\":[${STEP_ID}]}}}")
STEP_HOLDER=$(echo "$R" | jq -r '.data.id')

fld_disable "$STEP_ID"
VER=$(fld_version "$STEP_ID")
# step 1→5（原来合法的 3 现在非法）
R=$(post "/fields/update" "{\"id\":${STEP_ID},\"label\":\"step 字段\",\"type\":\"integer\",\"category\":\"basic\",\"properties\":{\"description\":\"\",\"expose_bb\":false,\"constraints\":{\"min\":0,\"max\":100,\"step\":5}},\"version\":${VER}}")
# 不标记为 bug，因为 step 语义上不算"范围约束"；但记录行为便于后续决策
CODE=$(echo "$R" | jq -r '.code' | tr -d '\r')
TOTAL=$((TOTAL + 1))
if [ "$CODE" = "0" ]; then
  echo "  [PASS] atk5.1 step 1→5 当前放行 (code=0) — 行为确认"
  PASS=$((PASS+1))
elif [ "$CODE" = "40007" ]; then
  echo "  [PASS] atk5.1 step 1→5 被拦截 (code=40007)"
  PASS=$((PASS+1))
else
  echo "  [FAIL] atk5.1 step 1→5 意外 code=$CODE"
  FAIL=$((FAIL+1))
fi

# ---- Attack 6: string.pattern 新增收紧不检查 ----
subsection "ATK-6: string.pattern 从空加到严格正则（收紧）应被拒绝"

R=$(post "/fields/create" "{\"name\":\"${P}atk_str\",\"label\":\"str\",\"type\":\"string\",\"category\":\"basic\",\"properties\":{\"description\":\"\",\"expose_bb\":false,\"constraints\":{\"minLength\":0,\"maxLength\":100}}}")
STR2=$(echo "$R" | jq -r '.data.id')
fld_enable "$STR2"

R=$(post "/fields/create" "{\"name\":\"${P}atk_str_holder\",\"label\":\"str持\",\"type\":\"reference\",\"category\":\"basic\",\"properties\":{\"description\":\"\",\"expose_bb\":false,\"constraints\":{\"refs\":[${STR2}]}}}")
STR2_HOLDER=$(echo "$R" | jq -r '.data.id')

fld_disable "$STR2"
VER=$(fld_version "$STR2")
R=$(post "/fields/update" "{\"id\":${STR2},\"label\":\"str\",\"type\":\"string\",\"category\":\"basic\",\"properties\":{\"description\":\"\",\"expose_bb\":false,\"constraints\":{\"minLength\":0,\"maxLength\":100,\"pattern\":\"^[0-9]+\$\"}},\"version\":${VER}}")
assert_bug "atk6.1 加 pattern 应 40007" "40007" "$R" "string 字段新增/修改 pattern 可能使已存数据失效，checkConstraintTightened 未覆盖 pattern。"

# ---- Attack 7: 空 refs 数组（reference 字段无目标）----
subsection "ATK-7: reference.refs=[] 无目标 — 当前允许，业务上应至少 1 个"

R=$(post "/fields/create" "{\"name\":\"${P}atk_empty_refs\",\"label\":\"空refs\",\"type\":\"reference\",\"category\":\"basic\",\"properties\":{\"description\":\"\",\"expose_bb\":false,\"constraints\":{\"refs\":[]}}}")
CODE=$(echo "$R" | jq -r '.code' | tr -d '\r')
TOTAL=$((TOTAL + 1))
if [ "$CODE" = "40017" ] || [ "$CODE" = "40014" ] || [ "$CODE" = "40000" ]; then
  echo "  [PASS] atk7.1 空 refs 被拒绝 (code=$CODE)"
  PASS=$((PASS+1))
else
  EMPTY_REF=$(echo "$R" | jq -r '.data.id // empty')
  echo "  [BUG ] atk7.1 reference.refs=[] 被允许创建 (code=$CODE)"
  FAIL=$((FAIL+1))
  BUGS+=("atk7.1: 允许创建空 refs 的 reference 字段，无业务意义；应在校验层拦截。")
  [ -n "$EMPTY_REF" ] && fld_rm "$EMPTY_REF"
fi

# ---- Attack 8: refs 含 0 / 负值 ----
subsection "ATK-8: refs 含 0/负值 — 应在格式层拦截"
...
if [ "$CODE" = "40014" ] || [ "$CODE" = "40000" ]; then
  echo "  [PASS] atk8.1 refs=[0] 被拒绝 (code=$CODE)"
  PASS=$((PASS+1))
else
  echo "  [BUG ] atk8.1 refs=[0] code=$CODE 未拦截"
  FAIL=$((FAIL+1))
  BUGS+=("atk8.1: refs 含 0 应视为无效。")
...
TOTAL=$((TOTAL + 1))
if [ "$CODE" = "40014" ] || [ "$CODE" = "40000" ]; then
  echo "  [PASS] atk8.2 refs=[-1] 被拒绝 (code=$CODE)"
  PASS=$((PASS+1))
else
  echo "  [BUG ] atk8.2 refs=[-1] code=$CODE 未拦截"
  FAIL=$((FAIL+1))
  BUGS+=("atk8.2: refs 含负值应视为无效。")
  ID_NEG=$(echo "$R" | jq -r '.data.id // empty')
  [ -n "$ID_NEG" ] && fld_rm "$ID_NEG"
fi

# ---- Attack 9: reference 字段改类型 → 非 reference，引用关系应清空 ----
subsection "ATK-9: reference → integer 类型变更 应清空 refs 引用"
...
assert_code "atk9.1 reference → integer 允许" "0" "$R"

RC_AFTER=$(fld_refcount "$MORPH_TGT")
TOTAL=$((TOTAL + 2))
[ "$RC_BEFORE" = "1" ] && { echo "  [PASS] atk9.2 类型变更前 tgt ref_count=1"; PASS=$((PASS+1)); } || { echo "  [FAIL] atk9.2 期望 1 实际 $RC_BEFORE"; FAIL=$((FAIL+1)); }
[ "$RC_AFTER" = "0" ]  && { echo "  [PASS] atk9.3 类型变更后 tgt ref_count=0（已清理）"; PASS=$((PASS+1)); } || { echo "  [BUG ] atk9.3 期望 0 实际 $RC_AFTER — Update 清理 refs 逻辑未执行"; FAIL=$((FAIL+1)); BUGS+=("atk9.3: reference→integer 后未减回 ref_count"); }

# ---- Attack 10: 模板纯顺序变化不应影响 field_refs ----
subsection "ATK-10: 模板纯排序变化不应影响字段 ref_count"
...
assert_code "atk10.1 纯反序 + required 变更成功" "0" "$R"

RC_A_AFTER=$(fld_refcount "$O_A")
RC_B_AFTER=$(fld_refcount "$O_B")
TOTAL=$((TOTAL + 2))
[ "$RC_A_BEFORE" = "$RC_A_AFTER" ] && { echo "  [PASS] atk10.2 O_A ref_count 不变 ($RC_A_BEFORE→$RC_A_AFTER)"; PASS=$((PASS+1)); } || { echo "  [BUG ] atk10.2 O_A ref_count 从 $RC_A_BEFORE 变为 $RC_A_AFTER"; FAIL=$((FAIL+1)); BUGS+=("atk10.2: 纯排序变化触发了 field_refs 操作"); }
[ "$RC_B_BEFORE" = "$RC_B_AFTER" ] && { echo "  [PASS] atk10.3 O_B ref_count 不变 ($RC_B_BEFORE→$RC_B_AFTER)"; PASS=$((PASS+1)); } || { echo "  [BUG ] atk10.3 O_B ref_count 从 $RC_B_BEFORE 变为 $RC_B_AFTER"; FAIL=$((FAIL+1)); BUGS+=("atk10.3: 纯排序变化触发了 field_refs 操作"); }

# 验证顺序确实翻转
R=$(tpl_detail "$O_TPL")
assert_field "atk10.4 fields[0]=O_B (反序成功)" ".data.fields[0].name" "${P}atk_ord_b" "$R"
assert_field "atk10.5 fields[0].required=true"  ".data.fields[0].required" "true" "$R"

# ---- Attack 11: 通用边界 / 注入 / 恶意输入 ----
subsection "ATK-11: 畸形输入与注入"

R=$(post "/fields/create" '{"name":"a]\"injection","label":"注入","type":"integer","category":"combat","properties":{}}')
assert_code "atk11.1 含特殊字符 name 40002" "40002" "$R"

R=$(post "/fields/create" "{\"name\":\"${P}sqli\",\"label\":\"'; DROP TABLE fields; --\",\"type\":\"integer\",\"category\":\"combat\",\"properties\":{}}")
CODE=$(echo "$R" | jq -r '.code' | tr -d '\r')
TOTAL=$((TOTAL + 1))
if [ "$CODE" = "0" ]; then
  echo "  [PASS] atk11.2 SQL-like label 被安全处理"; PASS=$((PASS+1))
  SQLI_ID=$(echo "$R" | jq -r '.data.id')
  fld_rm "$SQLI_ID"
else
  echo "  [FAIL] atk11.2 意外 code=$CODE"; FAIL=$((FAIL+1))
fi
...
assert_code "atk11.3 超长 name 40002" "40002" "$R"

R=$(curl -s -X POST "$BASE/fields/create" -H "Content-Type: application/json" -d '{bad json}')
assert_code "atk11.4 畸形 JSON 40000" "40000" "$R"

R=$(curl -s -X POST "$BASE/fields/create" -H "Content-Type: application/json" -d '')
assert_code "atk11.5 空请求体 40000" "40000" "$R"

# properties 传数组而非对象
R=$(post "/fields/create" "{\"name\":\"${P}arr_props\",\"label\":\"x\",\"type\":\"integer\",\"category\":\"combat\",\"properties\":[]}")
CODE=$(echo "$R" | jq -r '.code' | tr -d '\r')
TOTAL=$((TOTAL + 1))
if [ "$CODE" = "40000" ]; then
  echo "  [PASS] atk11.6 properties=[] 拒绝 (40000)"; PASS=$((PASS+1))
else
  # 可能 Go json 能把 [] unmarshal 到 FieldProperties 的空值 → 成功
  ARR_ID=$(echo "$R" | jq -r '.data.id // empty')
  if [ -n "$ARR_ID" ] && [ "$ARR_ID" != "null" ]; then
    echo "  [BUG ] atk11.6 properties=[] 被接受 (code=$CODE)"
    FAIL=$((FAIL+1))
    BUGS+=("atk11.6: properties 字段类型约束不严格，数组被接受。")
    fld_rm "$ARR_ID"
  else
    echo "  [PASS] atk11.6 properties=[] 被拒绝 (code=$CODE)"; PASS=$((PASS+1))
  fi
fi

# 模板 field_id=0 / 负值
R=$(post "/templates/create" "{\"name\":\"${P}atk_fz\",\"label\":\"x\",\"description\":\"\",\"fields\":[{\"field_id\":0,\"required\":true}]}")
assert_code "atk11.7 field_id=0 40000" "40000" "$R"

R=$(post "/templates/create" "{\"name\":\"${P}atk_fn\",\"label\":\"x\",\"description\":\"\",\"fields\":[{\"field_id\":-1,\"required\":true}]}")
assert_code "atk11.8 field_id=-1 40000" "40000" "$R"

R=$(curl -s -X POST "$BASE/templates/create" -H "Content-Type: application/json" -d '{bad json}')
assert_code "atk11.9 模板畸形 JSON 40000" "40000" "$R"

R=$(curl -s -X POST "$BASE/templates/create" -H "Content-Type: application/json" -d '')
assert_code "atk11.10 模板空 body 40000" "40000" "$R"
...
assert_code "atk11.11 超长模板 name 41002" "41002" "$R"

# ---- Attack 12: 极端数字 ----
subsection "ATK-12: 极端数字约束（int 溢出/大于 int64）"

# integer 用 max=9999999999999999999 (> int64)
...
CODE=$(echo "$R" | jq -r '.code' | tr -d '\r')
TOTAL=$((TOTAL + 1))
echo "  [INFO] atk12.1 超大 int 约束 code=$CODE — 行为确认"
if [ "$CODE" = "0" ]; then
  BIG_ID=$(echo "$R" | jq -r '.data.id')
  PASS=$((PASS+1))
  fld_rm "$BIG_ID"
else
  PASS=$((PASS+1))   # 任一结果都可接受，只是确认
fi
...
# 清理攻击测试创建的字段和模板
for ID in "$O_TPL"; do
  [ -n "$ID" ] && [ "$ID" != "null" ] && tpl_rm "$ID"
done

for ID in "$HOLDER" "$FL_HOLDER" "$STEP_HOLDER" "$STR2_HOLDER" "$REF_B" "$REF_A" "$MORPH" \
          "$SEL_ID" "$FL_ID" "$STEP_ID" "$STR2" "$MORPH_TGT" \
          "$O_A" "$O_B" "$LEAF1" "$LEAF2" \
