...
```
{
  id, name, display_name, perception_mode, enabled, version, created_at, updated_at,
  config: { display_name, default_severity, default_ttl, perception_mode, range, <扩展字段...> },
  extension_schema: [
    { field_name, field_label, field_type, constraints, default_value, sort_order, enabled },
    ...
  ]
}
```

**注意**：`perception_mode` 同时出现在顶层和 `config` 中。顶层是 DB 列直出，供前端快速判断；`config` 里是 `config_json` 展开结果，供导出一致性。
...
写入使用乐观锁 `UPDATE ... WHERE id=? AND version=?`，rows=0 返回 `errcode.ErrVersionConflict`（哨兵错误定义在 `errcode/store_errors.go`），Service 层转 `42010 ErrEventTypeVersionConflict`。
...
- **Handler**：调用完整 `h.checkName()` 做正则 + 长度校验（非空 + `^[a-z][a-z0-9_]*$` + 长度上限），与创建接口一致。
...
| Handler | `EventTypeHandler.CheckName` — 调用 `h.checkName()` 完整格式校验 |
