...
> **实现状态**：**规划中**。需要先完成游戏服务端 `EventTypeConfig` struct 的兼容改造（见"前置依赖"），再开始 ADMIN 侧开发。
> 本文档是「用户场景 → 校验 → 调用链」的按功能展开说明；存储/缓存策略见 [backend.md](backend.md)，UI 流程/组件树见 [frontend.md](frontend.md)。
> 事件类型是 ADMIN 对"游戏世界里会发生什么事"的元数据登记。字段分成**系统字段**（硬编码）和**扩展字段**（运营通过 Schema 管理页自定义）两类。
> **单存储**：事件类型只存 MySQL，**不走 MongoDB**。导出 API 直接输出 `event_types.config_json` 列给游戏服务端（见 [../../api-contract.md](../../api-contract.md) 的 event_types 段）。
...
详见根目录对话记录中给服务端 CC 的契约说明，以及 [api-contract.md](../../api-contract.md) 的 event_types 段。
...
经与服务端 CC 协商确认，ADMIN 侧对扩展字段值做类型和约束校验后才落库。服务端运行时会对类型不匹配做"退化到 `defaults.go` 默认值 + slog.Warn"的兜底，但**不作为常规路径**——触发 warn 意味着 ADMIN 侧有 bug（校验漏了、Schema 迁移出错、或直接绕过 Service 层写 MySQL）。
...
**场景 — 管理员定义一个新事件（"枪声"、"地震"）。** 填写系统字段 + 扩展字段后提交，默认**未启用**。
...
- **Handler**（`EventTypeHandler.Create`）做格式/必填校验：
  - `name` 符合 `identPattern = ^[a-z][a-z0-9_]*$`，长度 ≤ `valCfg.EventTypeNameMaxLength`
  - `display_name` 非空，长度 ≤ `valCfg.EventTypeLabelMaxLength`
  ...
  - `extensions` 必须是 JSON 对象形状（拦 `null` / 数组 / 标量）
...
- **Service**（`EventTypeService.Create`）做业务校验：
  ...
  - 对 `extensions` 里每个 key 查 `EventTypeSchemaCache`：
    ...
    - 值符合 schema 定义的 `constraints`，不符合 `42007 ErrEventTypeExtValueInvalid`（复用字段管理抽出来的 `constraint.ValidateValue`）
...
**等 FSM/BT 上线后的补充**：删除事务内要加 `FOR SHARE` 锁 `event_type_refs` 防 TOCTOU（与字段管理功能 5 同构）。本期简化处理。
...
**场景 — 在事件类型管理页新建时，管理员输入 `name` 后失焦，前端实时告知这个名字能不能用。**
...
**场景 — 运营/管理员在 Schema 管理页的"事件类型扩展字段"tab 浏览所有已定义的扩展字段。**
...
**场景 — 运营想给所有事件类型加一个新字段 `priority (int, 1-10)`，让今后新建事件时表单多一个输入框。**
...
- **Handler**：`field_name` 正则 / `field_label` 长度 / `field_type` 枚举 / `constraints` JSON 对象形状 / `default_value` 非空
- **Service**：
...
**场景 — 运营修改某个扩展字段的 label / constraints / default / sort_order。**
...
**为什么不拦截收紧**：字段管理能拦截是因为 `ref_count` 能精确反映引用方数量；这里"被引用"意味着"某条事件类型的 config_json 里有这个 key"，查询代价是全表 JSON 扫描。两条路：

1. 扫描拦截（严谨但贵）
2. 放行 + 前端弹窗提示（务实）

本期选 2。真要严格，未来可以加个"影响评估"按钮，跑 `SELECT COUNT(*) WHERE JSON_CONTAINS_PATH(config_json, 'one', '$.xxx')` 估算影响面，但不强制阻断。
...
- 只导 `enabled=1 AND deleted=0` 的记录（这个策略本期就定，不走延后功能 48）
...
| 42031 | `ErrExtSchemaEditNotDisabled` | 编辑前必须先停用（可选，看是否照搬字段管理） |
...
| 系统字段热更新 | 无，服务端启动时一次性加载 | 服务端未来加热加载接口（可能走 `POST /api/runtime/reload-event-types` 或变更事件订阅，届时对齐）|
| 服务端默认值对照 API | 无 | 未来可加 `GET /api/runtime/event-type-defaults`，Schema 管理页直接展示"服务端当前默认值 vs 运营设置的 default_value"对照表 |
