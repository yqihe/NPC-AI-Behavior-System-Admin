...
- `Enabled` 三态：`nil` 不筛选、`true` 仅启用、`false` 仅停用。
...
在字段管理页，管理员要定义一个新的 NPC 属性（比如"生命值"、"阵营"）。填写字段标识、中文标签、类型、分类和动态属性后提交。
...
- **Handler**（`FieldHandler.Create`）做格式/必填校验：
  - `name` 符合 `identPattern = ^[a-z][a-z0-9_]*$`、长度 ≤ `valCfg.FieldNameMaxLength`
  - `label` 非空且长度 ≤ `valCfg.FieldLabelMaxLength`
  - `type` / `category` 非空
  - `properties` 必须是 JSON 对象形状（首字符 `{`，拦 `null` / 数组 / 标量）
- **Service**（`FieldService.Create`）做业务校验：
  - 字典存在性（`checkTypeExists` / `checkCategoryExists`）
  - `name` 唯一性（含软删除）
  - 对 `reference` 类型调 `validateReferenceRefs`（非空 / 存在 / 启用 / 非嵌套 / 无循环）
...
| 错误码 | 常量 | 触发场景 |
|--------|------|---------|
| 40001 | `ErrFieldNameExists` | `name` 已存在（含软删除） |
| 40002 | `ErrFieldNameInvalid` | `name` 格式不合法或长度超限 |
| 40003 | `ErrFieldTypeNotFound` | `type` 不在字典中 |
| 40004 | `ErrFieldCategoryNotFound` | `category` 不在字典中 |
| 40009 | `ErrFieldCyclicRef` | reference 类型检测到循环引用 |
| 40013 | `ErrFieldRefDisabled` | reference 引用了已停用的字段 |
| 40014 | `ErrFieldRefNotFound` | reference 引用的字段不存在 |
| 40016 | `ErrFieldRefNested` | reference 引用了另一个 reference（禁嵌套） |
| 40017 | `ErrFieldRefEmpty` | reference 类型 `refs` 为空 |
| 40000 | `ErrBadRequest` | `label` / `type` / `category` / `properties` 必填缺失或格式错 |

### 边界 case

- `name` 包含软删除记录也不可复用。
- reference 字段的主记录 `FieldStore.Create` 和 `syncFieldRefs` 不在同一事务中（已知瑕疵，极端情况可能不一致）。
...
| 错误码 | 常量 | 触发场景 |
|--------|------|---------|
| 40011 | `ErrFieldNotFound` | 字段不存在（或命中空标记） |
| 40000 | `ErrBadRequest` | `id` 不合法 |

### 边界 case

- 停用字段也能查详情。
- Redis 获取锁失败时降级直查 MySQL。
...
在字段管理页，管理员要修改字段的标签、类型、分类或约束。只有**未启用**状态才能编辑——启用中的字段已对外可见，允许随意编辑会导致引用方看到不稳定的配置。试图编辑启用中的字段返回 `40015 ErrFieldEditNotDisabled`。
...
- **Handler**（`FieldHandler.Update`）：`id > 0` / `label` 非空且长度 ≤ 上限 / `type` / `category` 非空 / `properties` JSON 对象形状 / `version > 0`
- **Service**（`FieldService.Update`）：字典校验 → `getFieldOrNotFound` → **enabled 必须为 false（40015）** → type 变更拦截（40006）→ `checkConstraintTightened`（40007）→ reference 校验（40013/40014/40016/40009）→ 乐观锁写入 → `syncFieldRefs` 同步引用关系 → 清缓存
...
| 40003 | `ErrFieldTypeNotFound` | `type` 不在字典中 |
| 40004 | `ErrFieldCategoryNotFound` | `category` 不在字典中 |
| 40006 | `ErrFieldRefChangeType` | 被引用且试图修改类型 |
| 40007 | `ErrFieldRefTighten` | 被引用且试图收紧约束 |
| 40009 | `ErrFieldCyclicRef` | reference 循环引用 |
| 40010 | `ErrFieldVersionConflict` | 乐观锁版本冲突 |
| 40011 | `ErrFieldNotFound` | 字段不存在 |
| 40013 | `ErrFieldRefDisabled` | 新增 reference 目标已停用 |
| 40014 | `ErrFieldRefNotFound` | 新增 reference 目标不存在 |
| 40015 | `ErrFieldEditNotDisabled` | 启用中不允许编辑 |
| 40016 | `ErrFieldRefNested` | reference 引用了另一个 reference |
| 40000 | `ErrBadRequest` | 格式/必填校验失败 |

### 边界 case

- `name` 不可修改。
- 类型从 reference 切换为其他类型时，旧引用关系整体清空。
- 已有引用目标后来被停用，仍允许保留（存量不动）。
- `syncFieldRefs` 与主记录更新不在同一事务（已知瑕疵）。
...
- **Handler**：`id > 0`
- **Service**：`getFieldOrNotFound` → `enabled=false` 校验 → 事务内 `HasRefsTx`（FOR SHARE）→ 软删 + 引用清理
...
- 事务内 `HasRefsTx` 用 `FOR SHARE` 防 TOCTOU 竞态。
- reference 类型字段删除时，同一事务内清理所有出向引用 + 递减被引用方 `ref_count`。
...
在字段管理页新建字段时，管理员输入字段标识后离开输入框，前端实时告知这个名字能不能用。
...
- 理论上引用方模板不可能缺失（因为字段被引用时模板不能被删），如果真的缺失 handler 会 `slog.Warn` 并保留空 label。
...
- **Service**：`getFieldOrNotFound` → 乐观锁更新。
...
在字段管理页新建或编辑字段时，"字段类型"和"标签分类"的下拉选项不是前端写死的，而是从后端动态获取。这样运营团队可以随时在字典表里加新类型、新分类，不需要改代码重新部署。
...
在字段管理页编辑字段时，如果这个字段已经被模板引用了，管理员想修改它的约束。比如把生命值的最大值从 100 改成 50。
...
内嵌于功能 4 的 Service 层，不独立暴露接口。

### 调用链

内嵌于 `FieldService.Update` 的 `checkConstraintTightened` 调用。

### 错误码

| 错误码 | 常量 | 触发场景 |
|--------|------|---------|
| 40007 | `ErrFieldRefTighten` | 被引用字段收紧了约束 |

### 边界 case

- 约束 key 命名必须前后端严格对齐，单一权威为 seed 文件 `backend/cmd/seed/main.go` 中 `field_type` 字典每条记录的 `constraint_schema`。
- constraint key 对照表：
...
**场景 A — 在字段管理页创建 reference 类型字段时，要从下拉列表选择它引用哪些字段。** 下拉列表走功能 1 的 `enabled=true` 筛选，只展示启用的字段。

**场景 B — 在字段管理页编辑一个已有的 reference 类型字段，想加几个新引用或去掉几个旧引用。** 对**新增**的引用，目标必须启用且不能是 reference 类型（规则 3/4）；但对**已有**的引用（在 `oldRefSet` 集合中），即使那个字段后来被停用、或历史上就是嵌套的 reference，也允许保留不拦截。

**场景 C — 在字段管理页删除一个 reference 类型字段时，它之前引用的那些字段的引用计数要减回去。** 这在删除事务内由 `RemoveBySource` + `DecrRefCountTx` 自动完成（功能 5）。

### 校验规则

Service 的 `validateReferenceRefs` 依次做 5 条校验：
...
内嵌于功能 2（创建）、功能 4（编辑）、功能 5（删除）的 Service 层。

相关实现：`service/field.go` 中的 `validateReferenceRefs` / `detectCyclicRef` / `syncFieldRefs` / `parseRefFieldIDs`。

### 错误码

| 错误码 | 常量 | 触发场景 |
|--------|------|---------|
| 40009 | `ErrFieldCyclicRef` | 循环引用（DFS 检测） |
| 40013 | `ErrFieldRefDisabled` | 新增目标字段已停用 |
| 40014 | `ErrFieldRefNotFound` | 目标字段不存在 |
| 40016 | `ErrFieldRefNested` | 目标是 reference 类型（禁嵌套） |
| 40017 | `ErrFieldRefEmpty` | `refs` 为空 |

### 边界 case

- **为什么禁止嵌套**：模板管理的前端设计假设 reference 字段只有"一层"——点开 popover 看到的子字段必然是 leaf（`integer / float / string / boolean / select`）。如果允许嵌套，popover 需要递归展开，成本很高。
- **循环检测**：`detectCyclicRef` 用 DFS 遍历 `currentID → newRefIDs → 每个 ref 的 refs → ...`，将 `currentID` 预先标记为已访问；遇到重复 ID 即返回 `40009`。
- **前端双重防御**：`FieldConstraintReference.vue` 的 `loadEnabledFields` 追加 `f.type !== 'reference'` 过滤，下拉从源头不展示其他 reference 字段；后端 `40016` 作为兜底。
- **主记录与引用关系非原子**：Create/Update 时主记录和 `syncFieldRefs` 不在同一事务中（已知瑕疵）。
...
| 方法 | 用途 | 事务归属 |
|------|------|---------|
| `ValidateFieldsForTemplate(ctx, fieldIDs)` | 模板创建/编辑时校验被勾选字段全部存在 + 启用 + 非 reference 类型。任一不存在返回 `41006`，任一停用返回 `41005`，任一为 reference 类型返回 `41012`。底层 `FieldStore.GetByIDs` 一次批量取 | 事务外预校验 |
...
由模板管理的 `TemplateHandler` 编排调用，不独立暴露 HTTP 接口。
...
| 41012 | `ErrTemplateFieldIsReference` | 勾选了 reference 类型字段 |

**重要原则**：`41005` / `41006` / `41012` 这三个错误码归在**模板段位**（41xxx），因为它们**由模板管理页消费**，与字段管理自身的 `40011` / `40013` / `40016` 语义不混用。

### 边界 case

- Service 层之间零依赖，所有跨模块编排都在 Handler 层。
