...
3. 如果 hasRefs=true:
   service.CheckConstraintTightened(ets.FieldType, ets.Constraints, req.Constraints, errcode.ErrExtSchemaRefTighten)
   → 收紧则返回 42028 拒绝
...
**约束收紧检查规则**（`service.CheckConstraintTightened`）：
