...
- **store**：纯 MongoDB 操作。每个 collection 一组方法（List/Get/Create/Update/Delete）。所有操作带 `context.WithTimeout`
- **cache**：Redis 封装。只缓存 List 结果（配置数量少，单条查询不缓存）。提供 Get/Set/Del 方法
...
#### BT 行为树（bt_trees）
- `type` 必须是合法节点类型：`sequence` / `selector` / `parallel` / `inverter` / `check_bb_float` / `check_bb_string` / `set_bb_value` / `stub_action`
