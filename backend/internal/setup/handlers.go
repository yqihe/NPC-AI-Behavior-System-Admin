...
func NewHandlers(st *Stores, svc *Services, mc *MemCaches, cfg *config.Config) *Handlers {
	return &Handlers{
		...
		FsmConfig:       handler.NewFsmConfigHandler(st.DB, svc.FsmConfig, svc.Field, svc.EventTypeSchema, svc.Npc, &cfg.FsmConfig),
		FsmStateDict:    handler.NewFsmStateDictHandler(svc.FsmStateDict, &cfg.FsmStateDict),
		BtTree:          handler.NewBtTreeHandler(st.DB, svc.BtTree, svc.Field, svc.EventTypeSchema, svc.Npc, &cfg.BtTree),
