...
type EntityType = 'field' | 'template' | 'event-type' | 'event-type-schema' | 'fsm-state-dict' | 'fsm-config' | 'bt-tree' | 'bt-node-type' | 'npc' | 'runtime-bb-key' | 'region'
...
const entityTypeLabel = computed(() => {
  ...
  if (entityType.value === 'region') return '区域'
  return '模板'
})
...
const reasonText = computed(() => {
  if (action.value === 'edit') {
    ...
    if (entityType.value === 'region') {
      return '已启用的区域会被游戏服务端导出接口返回，修改期间可能导致服务端拉取到不稳定配置。请先禁用，再进入编辑。'
    }
...
async function onActOnce() {
  ...
  try {
    ...
    } else if (entityType.value === 'region') {
      const detail = await regionApi.detail(id)
      await regionApi.toggleEnabled(id, false, detail.data.version)
    } else {
    ...
    if (entityType.value === 'field') {
      ...
      conflictCode = REGION_ERR.VERSION_CONFLICT
    } else {
